...
class Game():
    ...
    def __init__(self, players, starting_player_idx, total_nr_of_stones, max_selection, verbose=True):        
        ...
        self.state = State(starting_player_idx, total_nr_of_stones)
    ...
    # Removes stones, checks if game is won and updates current player
    def select_stones(self, nr_of_stones):     
        self.remove_stones(nr_of_stones)
        self.check_if_won()  
        self.state.current_player_idx = self.next_player_idx(self.state.current_player_idx)
    ...
    def remove_stones(self, nr_of_stones):
        ...
        if self.verbose:
            print("[P{0} - {1}] selects {2} stone{3}: Remaining stones = {4}"
                .format(self.state.current_player_idx + 1, self.players[self.state.current_player_idx].name, nr_of_stones,
                    "s" if nr_of_stones > 1 else "", self.state.remaining_stones))
    
    def check_if_won(self):
        if self.state.remaining_stones == 0:
            self.winning_player_idx = self.state.current_player_idx
            self.over = True
            if self.verbose:
                print("[P{0} - {1}] wins!\n".format(self.state.current_player_idx + 1, self.players[self.state.current_player_idx].name))
    ...
    def user_input(self):
        inpt = input("[P{0} - {1}] Select stones: ".format(self.state.current_player_idx + 1, self.players[self.state.current_player_idx].name))
        if inpt == "exit" : exit()
